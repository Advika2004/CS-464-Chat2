	...
	if (strcasecmp(firstChunk, "%M") == 0) { 
       char *handle = strtok(NULL, " "); 
		if (handle == NULL) {
            ...
            printf("%%M <destination handle> <text>\n");
            return NULL;
        }
	   chunks[i] = handle;  
	   i++;
       ...
       char *number = strtok(NULL, " ");
		if (number == NULL) {
        	...
        	printf("%%C <num-handles> <destination-handle> <destination-handle> <text>\n");
        	return NULL;
    	}
	   chunks[i] = number;
	   i++;
...
//will create the right PDU's using the chunks array based on the type
void sendToServer(int socketNum)
{
	uint8_t sendBuf[MAX_MESSAGE_LENGTH];
	int sendLen = 0;
	int sent = 0;
	...
	if (chunkArray[0] == NULL) {
		...
		//this loop will send it in chunks of 200
		...
		//get out how many people to send it to
		int numDestHandles = atoi(chunkArray[1]);
		//where in the chunks array the message is stored
    	...
    	for (int i = 0; i < chunksToSend; i++) {
        	...
        	memcpy(newChunk, message + twoHunnidOffset, howMuchToCopy);
        	newChunk[howMuchToCopy] = '\0';
        	// make the new chunk the message part in the chunks array
...
void receiveHandleConf(int socketNum){
    ...
    uint8_t responseFlag = responseBuf[0];
    if (responseFlag == 2) {
